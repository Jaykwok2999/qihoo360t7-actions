...
sed -i "s/NAME=\"*.*\"/NAME=\"OpenWrt\"/g" package/base-files/files/usr/lib/os-release
sed -i "s/BUILD_ID=\"*.*\"/BUILD_ID=\"$(date +%Y%m%d%H) By JayKwok\"/g" package/base-files/files/usr/lib/os-release
